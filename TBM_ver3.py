...
class TightBindingModel_VQE:
    def __init__(self, num_q_orbital, num_q_site, hamiltonian_dictionary, simulation_mode):
        self.num_q_orbital=num_q_orbital
        self.num_q_site=num_q_site
    ...
    def ansatze_site(self, p):
        ...
        # qc_ansatze_site.append(QFT(self.num_q_site, name=f'QFT[p={p}]'), range(self.num_q_site))
        ...
        if previous_params_list==None: # target energy level is 0th : ground state band
            ...
            for p in p_list:
                ...
                if self.simulation_mode == 'noisy':
                    # qc_site_0th = transpile(qc_site_0th, basis_gates=['cx', 'h', 't', 'tdg'], optimization_level=2)
                    # qc_orbital_0th = transpile(qc_orbital_0th, basis_gates=['cx', 'h', 't', 'tdg'], optimization_level=2)
                ...
                if self.simulation_mode == 'ideal':
                    # qc_site_0th=qc_site_0th
                    # qc_orbital_0th=qc_orbital_0th
                    estimator_site = Estimator()
                
                # estimator_site = Estimator()
                
                job_A_N = estimator_site.run([(qc_site_0th, A_N)])
                result_A_N = job_A_N.result()[0]
                exp_A_N=float(result_A_N.data.evs)
                
                job_B_N = estimator_site.run([(qc_site_0th, B_N)])
                result_B_N = job_B_N.result()[0]
                exp_B_N=float(result_B_N.data.evs)
                ...
                term_0=A_M_0
                term_A_1=A_M_1*(exp_A_N)
                term_B_1 = B_M_1*(-exp_B_N)
            ...
            for p in p_list:
                ...
                if self.simulation_mode == 'noisy':
                    # qc_site_ith = transpile(qc_site_ith, basis_gates=['cx', 'h', 't', 'tdg'], optimization_level=2)
                    # qc_orbital_ith = transpile(qc_orbital_ith, basis_gates=['cx', 'h', 't', 'tdg'], optimization_level=2)
                    ...
                    estimator_site = BackendEstimatorV2(backend=simulator_backend)
                if self.simulation_mode == 'ideal':
                    # qc_site_ith=qc_site_ith
                    # qc_orbital_ith=qc_orbital_ith
                    estimator_site = Estimator()
                
                # estimator_site = Estimator()
                
                job_A_N = estimator_site.run([(qc_site_ith, A_N)])
                result_A_N = job_A_N.result()[0]
                exp_A_N=float(result_A_N.data.evs)
                
                job_B_N = estimator_site.run([(qc_site_ith, B_N)])
                result_B_N = job_B_N.result()[0]
                exp_B_N=float(result_B_N.data.evs)
